    ...
    MyBot.prototype.onDisputing = function (orderSet, snapshot) {
        try {
            ...
            if (this.isINear(myRegion, ballRegion)) {
                var myDist = lugo4node_1.geo.distanceBetweenPoints(me.getPosition(), ballPosition);
                var closerPlayers = 0;
                for (var _i = 0, _b = reader.getMyTeam().getPlayersList(); _i < _b.length; _i++) {
                    var player = _b[_i];
                    if (player.getNumber() == me.getNumber() && player.getNumber() == 1)
                        continue;
                    var playerDist = lugo4node_1.geo.distanceBetweenPoints(player.getPosition(), ballPosition);
