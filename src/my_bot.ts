...
import { getMyExpectedPosition, MAPPER_ROWS } from './settings';
    ...
    onDisputing(orderSet: Lugo.OrderSet, snapshot: Lugo.GameSnapshot): Lugo.OrderSet {
        try {
            ...
            if (this.isINear(myRegion, ballRegion)) {
                const myDist = geo.distanceBetweenPoints(me.getPosition(), ballPosition);
                let closerPlayers = 0;

                for (const player of reader.getMyTeam().getPlayersList()) {
                    if (player.getNumber() == me.getNumber() && player.getNumber() == 1) continue;
                    const playerDist = geo.distanceBetweenPoints(player.getPosition(), ballPosition);
                    if (playerDist < myDist) {
                        closerPlayers++;
                        if (closerPlayers >= 2) break;
                    }
                }
                if (closerPlayers < 2) {
